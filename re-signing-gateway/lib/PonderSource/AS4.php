...
class AS4 {
  public function handleAs4($contentType, $body) {
    ...
    $WHITELIST = [
      "9915:phase4-test-sender" => file_get_contents(dirname(dirname(dirname(__FILE__))) . '/p12transport/sender.cer')
    ];
		...
		$keystore_file = dirname(dirname(dirname(__FILE__))) . '/p12transport/test.p12'; // Private key of the receiver/us
  ...
  public function as4SendWithIdentifier($invoice, $receiver_identifier) {
		...
		if ($useSMP) {
			...
			$as4_endpoint = 'http://mesh.pondersource.org:8080/as4'; // server from scripts/start-to-Phase4.sh inside Docker testnet
			$cert_file = dirname(dirname(dirname(__FILE__))) . '/p12transport/receiver.cer';
		...
		$keystore_file = dirname(dirname(dirname(__FILE__))) . '/p12transport/test.p12';

...
class UBLCountries {
    ...
    const COUNTRIES = [
        ...
        'GA' => 'Gabon', 'GB' => 'United Kingdom of Great Britain and Northern Ireland', 'GD' => 'Grenada',
        ...
        '1A' => 'Kosovo', 'XI' => 'United Kingdom (Northern Ireland)'
    ];

    public static function findCountry(string $name): array {
        $candid = array_filter(self::COUNTRIES, function ($fullName, $code) use($name) {
            return strtoupper($fullName) === strtoupper($name) ||
                    strtoupper($code) === strtoupper($name);
        ...
        if (count($candid) > 0) {
            ...
            return [
                'code' => $key,
                'name' => self::COUNTRIES[$key]
            ];
        }

        $matches = [];

        foreach (self::COUNTRIES as $code => $fullName) {
            if (strpos($fullName, $name) !== false) {
                $matches[$code] = strlen($fullName);
            }
        }
